...
{
    public class GattCharacteristic : BluetoothLE.Server.AbstractGattCharacteristic, IAppleGattCharacteristic
    {
        readonly CBPeripheralManager manager;
        readonly IDictionary<NSUuid, BluetoothLE.Server.IDevice> subscribers;
        public CBMutableCharacteristic Native { get; }


        public GattCharacteristic(CBPeripheralManager manager,
                                  BluetoothLE.Server.IGattService service,
                                  Guid characteristicUuid,
                                  CharacteristicProperties properties,
                                  BluetoothLE.Server.GattPermissions permissions) : base(service, characteristicUuid, properties, permissions)
        {
            this.manager = manager;
            this.subscribers = new ConcurrentDictionary<NSUuid, BluetoothLE.Server.IDevice>();
        ...
        public override IReadOnlyList<BluetoothLE.Server.IDevice> SubscribedDevices
        {
            get
            {
                lock (this.subscribers)
                {
                    return new ReadOnlyCollection<BluetoothLE.Server.IDevice>(this.subscribers.Values.ToArray());
                }
            }
        }


        public override void Broadcast(byte[] value, params BluetoothLE.Server.IDevice[] devices)
        {
        ...
        public override IObservable<CharacteristicBroadcast> BroadcastObserve(byte[] value, params BluetoothLE.Server.IDevice[] devices)
        ...
        protected override BluetoothLE.Server.IGattDescriptor CreateNative(Guid uuid, byte[] value) => new GattDescriptor(this, uuid, value);


        protected virtual EventHandler<CBPeripheralManagerSubscriptionEventArgs> CreateSubHandler(IObserver<BluetoothLE.Server.DeviceSubscriptionEvent> ob, bool subscribing)
            => (sender, args) =>
            {
        ...
        BluetoothLE.Server.IDevice GetOrAdd(CBCentral central)
        {
            lock (this.subscribers)
            {
        ...
        BluetoothLE.Server.IDevice Remove(CBCentral central)
        {
            lock (this.subscribers)
            {
                if (this.subscribers.ContainsKey(central.Identifier))
